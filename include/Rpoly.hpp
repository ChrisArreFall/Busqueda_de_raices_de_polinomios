...
class RPoly {
public:
    int findRoots(const std::vector<T> &op, int degree, std::vector<T> &zeror, std::vector<T> &zeroi);
private:
 ...
 */
    template <class T>
    void RPoly<T>::nextk(const int *type)
    {
        T temp;
        int i;
...
//to avoid compiler error on types that are not going to exist anyway
namespace Jenkins {

    template <>
    class RPoly<dcomp> {
    public:
        int findRoots(const std::vector<dcomp> &op, int degree, std::vector<dcomp> &zeror, std::vector<dcomp> &zeroi){return 0;}

    };
    template <>
    class RPoly<fcomp> {
    public:
        int findRoots(const std::vector<fcomp> &op, int degree, std::vector<fcomp> &zeror, std::vector<fcomp> &zeroi){return 0;}
    };
