...
class CPoly {
public:
    int findRoots( const T *opr, const T *opi, int degree, std::vector<T> &zeror, std::vector<T> &zeroi );
private:
    T sr, si, tr, ti, pvr, pvi, are, mre, eta, infin;
    int nn;
    std::vector<T> pr, pi, hr, hi, qpr, qpi, qhr, qhi, shr, shi;
    ...
    void polyev( const int nn, const T sr, const T si, std::vector<T> & pr, std::vector<T> & pi, std::vector<T> & qr, std::vector<T> & qi, T *pvr, T *pvi );
    T errev( const int nn, std::vector<T> & qr, std::vector<T> & qi, const T ms, const T mp, const T are, const T mre );
    void cauchy( const int nn, std::vector<T>& pt, std::vector<T>& q, T *fn_val );
    T scale( const int nn, std::vector<T> & pt, const T eta, const T infin, const T smalno, const T base );
...
namespace JT {
    ...
    template<class T>
    int CPoly<T>::findRoots( const T *opr, const T *opi, int degree,  std::vector<T> &zeror,  std::vector<T> &zeroi )
    {
        ...
        // Allocate arrays
        pr = std::vector<T>(degree+1);
        pi = std::vector<T>(degree+1);
        hr = std::vector<T>(degree+1);
        hi = std::vector<T>(degree+1);
        qpr= std::vector<T>(degree+1);
        qpi= std::vector<T>(degree+1);
        qhr= std::vector<T>(degree+1);
        qhi= std::vector<T>(degree+1);
        shr= std::vector<T>(degree+1);
        shi= std::vector<T>(degree+1);
...
//
    template<class T>
    void CPoly<T>::polyev( const int nn, const T sr, const T si,std::vector<T> & pr, std::vector<T> & pi, std::vector<T> & qr, std::vector<T> & qi, T *pvr, T *pvi )
    {
        int i;
        T t;
...
//
    template<class T>
    T CPoly<T>::errev( const int nn, std::vector<T> & qr, std::vector<T> &qi, const T ms, const T mp, const T are, const T mre )
    {
        int i;
        T e;
...
//
    template<class T>
    void CPoly<T>::cauchy( const int nn, std::vector<T>& pt, std::vector<T>& q, T *fn_val )
    {
        int i, n;
        T x, xm, f, dx, df;
...
//
    template<class T>
    T CPoly<T>::scale( const int nn, std::vector<T>& pt, const T eta, const T infin, const T smalno, const T base )
    {
        int i, l;
